...
# Isolate the predictor variables from the COMPLETE dataset
...
# Create the Survival response object from the SAME complete dataset
y_surv <- Surv(
...
```{r}
# Identify only the genes that LASSO selected
selected_genes <- active_features[active_features %in% gene_cols]

# Build a new dataframe using the ORIGINAL clinical columns + the winning genes
...
```{r}
# Generate the risk scores (linear predictors) from your final model
final_model_data_clean$risk_score <- predict(clean_cox_model, type = "lp")

# Define the time horizon (same as your calibration plot)
time_horizon <- quantile(final_model_data_clean$exit_time, 0.80)
...
train_raw <- nsclc[train_idx, ]
test_raw <- nsclc[-train_idx, ]
